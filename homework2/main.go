...
func main() {
	...
	go func() {
		for {
			changeDepth(crawler.setMaxDepth, 2)
		}
	}()
...
type setDepth func(int)

func changeDepth(fs setDepth, depth int) {
	osSignalChan := make(chan os.Signal)
	signal.Notify(osSignalChan, syscall.SIGUSR1)
	<-osSignalChan
	fs(depth)
	log.Printf("set depth")
}
